...
export class BillService {
  ...
  public get() {
    return this.http.get(this.accessPointUrl, { headers: this.headers }).pipe(
      retry(2),
      map((data: any[]) =>
        data.map((item: any) => this.adapter.fromJsonToModel(item))
      )
    );
  }
  ...
  public getUnpaidBills() {
    return this.http.get(this.accessPointUrl + "/unpaid", {
  ...
  public downloadLogs() {
    return this.http
      .get(this.accessPointUrl + "/download", { responseType: "blob" })
      .pipe(retry(2));
  }
  ...
  public postBillsForSelectedMonth(monthByName: string) {
    debugger;
    return this.http
      .post(this.accessPointUrl, JSON.stringify(monthByName), {
        headers: this.headers,
      })
      .pipe(
        retry(2),
  ...
  public remove(bill: Bill) {
    return this.http
      .delete(this.accessPointUrl + "/" + bill.id, { headers: this.headers })
      .pipe(retry(2));
  }
  ...
  public update(bill: Bill) {
    return this.http
      .put(
        this.accessPointUrl + "/" + bill.id,
        this.adapter.fromModelToJson(bill),
        { headers: this.headers }
      )
      .pipe(retry(2));
  }
